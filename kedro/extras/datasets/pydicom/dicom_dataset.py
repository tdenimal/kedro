...
"""``DICOMDataSet`` loads a DICOM ( Digital Imaging and Communications in Medicine ) datafile
using an underlying filesystem (e.g.: local, S3, GCS) and extracts metadata info and image as pandas DataFrame and numpy array respectively.
It uses the ``pydicom`` package to manage dcm files.
"""
...
class DICOMDataSet(
    ...
    Note that ``DICOMDataSet`` doesn't handle saving dcm files.

    Example:
    ::

        >>> from kedro.extras.datasets.pydicom import DICOMDataSet
        >>>
    ...
    def __init__(
        ...
        """Creates a new instance of ``DICOMDataSet`` pointing to a concrete dcm file
        on a specific filesystem.

        Args:
            filepath: Filepath in POSIX format to a dcm file prefixed with a protocol like `s3://`.
